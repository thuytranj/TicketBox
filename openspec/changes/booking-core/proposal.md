...
| R4 | **Cổng thanh toán không ổn định** | VNPAY/MoMo có thể timeout, trả lỗi 5xx, hoặc gọi webhook callback nhiều lần. Nếu không xử lý đúng, hệ thống sẽ bị treo chờ response hoặc trừ tiền 2 lần. | **Circuit Breaker** (thư viện `opossum`) tự động cắt mạch khi tỷ lệ lỗi > 50% trong 10 giây. **Idempotency Key** trên Redis (TTL 24h) đảm bảo mỗi giao dịch chỉ xử lý tối đa 1 lần, dù webhook bị gọi lặp. |
