...
9. Sinh mã QR an toàn cho mỗi khách mời bằng HMAC-SHA256 kết hợp payload định danh (`guestId:concertId`) và `SERVER_SECRET`.
10. Đẩy tin nhắn gửi email thư mời đính kèm mã QR sang exchange/queue notification. Email worker tiêu thụ hàng đợi với cấu hình `prefetch = 1` và bộ điều tiết tốc độ (delay 100-200ms trước mỗi lần gửi) để khống chế lưu lượng gửi (Rate Limiting Consumer) tối đa 5-10 email/giây.
