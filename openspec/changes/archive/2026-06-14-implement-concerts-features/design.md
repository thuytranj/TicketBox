...
- `poster_public_id`: `varchar(255)` (Có thể rỗng, lưu trữ định danh công khai của ảnh trên Cloudinary phục vụ dọn dẹp khi cập nhật/xóa)
- `summary`: `text` (Có thể rỗng, sinh bằng AI)
- `tags`: `varchar(50)[]` (Không rỗng, mặc định là mảng rỗng `{}`)
- `svg_stage_map`: `text` (Có thể rỗng, lưu trữ bản đồ sơ đồ ghế ngồi/sân khấu dạng chuỗi SVG được đọc và xử lý từ Frontend)
...
- **Xóa (Delete)**:
  ...
  - **Nếu chưa có booking nào**: 
    - Đầu tiên, hệ thống sẽ thực hiện xóa file ảnh cũ trên Cloudinary thông qua `posterPublicId` (nếu có).
    - Thực hiện xóa vật lý (hard delete) concert khỏi PostgreSQL, đồng thời xóa cache `cache:concerts:{id}` và `cache:concerts:{id}:stagemap` trong Redis. Do thiết kế khóa ngoại ở bảng `ticket_types` sử dụng `ON DELETE CASCADE`, toàn bộ các loại vé liên kết sẽ tự động bị xóa theo.
...
- **Trả về**: JSON chứa cả `url` (đường dẫn ảnh an toàn) và `publicId` (mã định danh lưu trữ của Cloudinary để quản lý dọn dẹp).
- **Multer Memory Storage**: Sử dụng bộ lưu trữ tạm thời trong RAM của Multer (`MemoryStorage`) để lấy đối tượng file (`file.buffer`).
- **Cloudinary Service**: Viết `CloudinaryService` đọc cấu hình `CLOUDINARY_CLOUD_NAME`, `CLOUDINARY_API_KEY`, `CLOUDINARY_API_SECRET` từ `.env`, tạo luồng Stream (`cloudinary.uploader.upload_stream`) để đẩy trực tiếp buffer ảnh lên Cloudinary (thư mục `ticketbox/posters`) mà không lưu tạm ra ổ đĩa cục bộ. Dịch vụ cũng cung cấp phương thức `deleteFile(publicId)` sử dụng `cloudinary.uploader.destroy` để thực hiện xóa ảnh cũ.
- **Ràng buộc**: Giới hạn file tải lên tối đa 10MB và chỉ chấp nhận định dạng ảnh phổ biến (`jpg`, `jpeg`, `png`, `webp`).
- **Dọn dẹp ảnh cũ (Cleanup)**:
  - Khi cập nhật concert (`PATCH /concerts/:id`): Nếu `posterUrl` hoặc `posterPublicId` được thay đổi hoặc xóa bỏ, hệ thống kiểm tra nếu `concert.posterPublicId` cũ tồn tại thì gọi `CloudinaryService.deleteFile` để xóa ảnh cũ trên Cloudinary.
  - Khi xóa concert (`DELETE /concerts/:id`): Nếu `concert.posterPublicId` cũ tồn tại, gọi `CloudinaryService.deleteFile` để xóa ảnh trước khi thực hiện xóa bản ghi DB.
...
- **Risk**: Độ trễ khi upload/xóa ảnh trên Cloudinary làm nghẽn luồng xử lý của server NestJS.
- **Biện pháp giảm thiểu**: Quá trình upload và xóa được thực hiện bất đồng bộ (sử dụng `Promise` và stream nhị phân hoặc API HTTP bất đồng bộ). Giới hạn kích thước ảnh tải lên ở mức 10MB để hạn chế tối đa độ trễ mạng và dung lượng truyền tải.
