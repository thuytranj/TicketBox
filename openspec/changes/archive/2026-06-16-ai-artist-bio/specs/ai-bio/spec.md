# Đặc tả: Tích hợp AI Artist Bio (AI Artist Bio Integration)

## Mô tả

Tính năng này tự động hóa quy trình tạo tiểu sử tóm tắt cho nghệ sĩ biểu diễn từ file PDF press kit do Ban tổ chức tải lên bằng cách sử dụng AI (Google Gemini). Quá trình xử lý được thực hiện bất đồng bộ qua RabbitMQ, cho phép xem bản nháp, tạo lại (regenerate) từ văn bản thô đã trích xuất, chỉnh sửa và xác nhận lưu chính thức vào Concert.

## Luồng chính

1. **Tải lên PDF và Khởi tạo**:
   - Ban tổ chức gửi yêu cầu `POST /concerts/:id/artist-bio` kèm file PDF.
   - Hệ thống trích xuất text từ file PDF (sử dụng `pdf-parse`), lưu text thô vào bảng `concert_ai_bios` và đặt trạng thái xử lý AI thành `processing`.
   - Gửi task `{ concertId, userId, rawText }` vào queue `ai.generate_bio` của RabbitMQ và trả về `202 Accepted` ngay lập tức.
2. **Sinh tiểu sử nháp (Background Worker)**:
   - Worker nhận task từ hàng đợi, gọi API Google Gemini (`gemini-3.5-flash`) để tóm tắt tiểu sử nghệ sĩ dưới 300 từ.
   - Nếu thành công, lưu bản nháp vào `draft_bio` và đặt trạng thái `completed` trong bảng `concert_ai_bios`, đồng thời chèn bản ghi thông báo thành công vào bảng `notification_logs`.
   - Nếu thất bại sau tối đa 3 lần thử lại, đặt trạng thái `failed` kèm thông tin lỗi vào `error` trong bảng `concert_ai_bios`, đồng thời chèn bản ghi thông báo lỗi vào bảng `notification_logs`.
3. **Xem trạng thái và bản nháp**:
   - Ban tổ chức gửi yêu cầu `GET /concerts/:id/artist-bio` để theo dõi trạng thái sinh và xem nội dung bản nháp tiểu sử nghệ sĩ.
4. **Tạo lại bản nháp (Regenerate)**:
   - Ban tổ chức gửi yêu cầu `POST /concerts/:id/artist-bio/regenerate` nếu chưa hài lòng với kết quả AI.
   - Hệ thống lấy trực tiếp `raw_text` đã lưu trong bảng `concert_ai_bios` để tạo task mới gửi lên RabbitMQ mà không bắt buộc tải lại file PDF.
5. **Duyệt và Công bố (Confirm)**:
   - Ban tổ chức gửi yêu cầu `PUT /concerts/:id/artist-bio/confirm` kèm nội dung tiểu sử mong muốn lưu (đã sửa hoặc giữ nguyên bản nháp).
   - Hệ thống lưu chính thức vào trường `biography` của bảng `concerts` và xóa toàn bộ cache Redis liên quan của concert đó.

## Kịch bản lỗi

- **Dữ liệu không hợp lệ**:
  - File tải lên không phải định dạng PDF hoặc dung lượng vượt quá 10MB -> Trả về lỗi `400 Bad Request`.
  - Concert ID không tồn tại -> Trả về lỗi `404 Not Found`.
- **Yêu cầu tạo lại khi chưa upload PDF**: Gọi API `/regenerate` nhưng hệ thống chưa có dữ liệu `raw_text` của concert đó -> Trả về lỗi `400 Bad Request`.
- **Lỗi kết nối API Gemini**: Worker tự động retry tối đa 3 lần với exponential backoff. Nếu lỗi vẫn tiếp tục sau 3 lần -> Cập nhật trạng thái `failed` kèm thông tin lỗi vào `error` và tạo thông báo lỗi In-app cho Admin.

## Ràng buộc

- **Bảo mật**: Chỉ các tài khoản có vai trò `organizer` mới có quyền truy cập và thực hiện các API này.
- **Tính nhất quán**: Khi công bố tiểu sử chính thức, toàn bộ cache Redis liên quan đến Concert đó phải bị vô hiệu hóa lập tức để đảm bảo thông tin hiển thị chính xác.
- **Giới hạn hiệu năng**: Kích thước file PDF tối đa là 10MB; tóm tắt tiểu sử tạo bởi AI được ràng buộc dưới 300 từ.

## Tiêu chí chấp nhận

- API trả về đúng mã trạng thái HTTP quy định (202 Accepted cho upload/regenerate, 200 OK cho confirm/get, 400/403/404 cho các kịch bản lỗi).
- Dữ liệu thô và bản nháp được lưu biệt lập ở bảng `concert_ai_bios`.
- Thông báo In-app xuất hiện chính xác trong bảng `notification_logs` với đúng trạng thái tương ứng.
- Sau khi confirm, biography hiển thị chính xác ở Concert và cache Redis được dọn sạch.
