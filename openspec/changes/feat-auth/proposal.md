...
- Cung cấp các API:
  ...
  - Yêu cầu gửi lại OTP kích hoạt (`POST /auth/resend-otp`)
...
- Gửi mã OTP xác thực và OTP reset mật khẩu qua email bất đồng bộ thông qua message broker RabbitMQ và SMTP mock (Mailtrap).
...
- `auth`: Cập nhật chi tiết các kịch bản đăng ký (pending), xác thực OTP kích hoạt tài khoản, gửi lại OTP, quên/đặt lại mật khẩu bằng OTP, đăng nhập, làm mới token, đăng xuất và chặn truy cập trái phép bằng RolesGuard.
...
- **Cache:** Sử dụng Redis để quản lý vòng đời Refresh Token và lưu trữ tạm thời mã OTP xác thực/OTP reset cùng các khóa Rate Limiter.
- **RabbitMQ:** Sử dụng RabbitMQ để đẩy task gửi email chứa mã OTP bất đồng bộ tới Worker xử lý gửi mail.
- **API:** Thêm controller và service mới cho module `/auth` hỗ trợ các cơ chế xác thực OTP, quên mật khẩu và quản lý phiên.
