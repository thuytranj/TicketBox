...
#### A. Cache Chi tiết Concert (Không kèm SVG)
...
- **Quy trình Lấy Chi tiết (Get Detail)**:
  ...
  2. Nếu tìm thấy (cache hit), giải tuần tự hóa chuỗi JSON (chứa thông tin concert kèm mảng `ticketTypes` và **không có trường `svg_stage_map`**) và trả về.
  3. Nếu không tìm thấy (cache miss), truy vấn dữ liệu từ PostgreSQL (dùng TypeORM select loại trừ cột `svg_stage_map` và load kèm quan hệ `ticketTypes`).
  4. Nếu tìm thấy trong DB, tuần tự hóa dữ liệu (concert + ticketTypes) thành chuỗi JSON và ghi vào Redis dưới khóa `cache:concerts:{id}` kèm theo TTL 10 phút, sau đó trả về kết quả.
  5. Nếu không tìm thấy trong DB, trả về lỗi 404.

#### B. Cache Sơ đồ Sân khấu (Chỉ chứa nội dung SVG)
...
#### C. Quy trình Thu hồi Cache (Cache Invalidation)
- Khi có bất kỳ thay đổi nào liên quan đến thông tin Concert (sửa thông tin, thay đổi trạng thái, xóa concert), hệ thống xóa cả hai khóa Redis `cache:concerts:{id}` và `cache:concerts:{id}:stagemap`.
- Khi có thay đổi liên quan đến loại vé `TicketType` (tạo loại vé mới, cập nhật giá/số lượng, xóa loại vé), hệ thống phải xác định `concert_id` liên kết và xóa khóa Redis `cache:concerts:{concert_id}` để danh sách vé được cập nhật mới nhất.
