...
3. API tạo bản ghi Job tracking trong cơ sở dữ liệu (`vip_guest_imports`) với trạng thái ban đầu là `pending`.
4. API gửi message chứa ID Job, ID concert và URL tệp CSV trên Supabase Storage vào hàng đợi RabbitMQ (`vip_guest.import`), rồi lập tức phản hồi HTTP `202 Accepted` kèm ID Job cho Client để thực hiện polling hoặc chờ thông báo real-time.
5. Worker nhận nhiệm vụ từ RabbitMQ, cập nhật trạng thái Job thành `processing`, tải tệp CSV từ Supabase Storage và thực hiện phân tích dạng stream (`csv-parser`).
...
7. Gom nhóm các dòng hợp lệ, kiểm tra trùng lặp email:
   - Nếu trùng lặp email với các dòng trước đó trong chính file CSV: ghi nhận lỗi vào danh sách `errorLogs` và bỏ qua.
   - Nếu trùng lặp email với khách VIP đã tồn tại trong DB của concert hiện tại: bỏ qua im lặng (`silently skip`) để hỗ trợ khôi phục lỗi dễ dàng khi upload lại tệp.
...
11. Sau khi hoàn thành hoặc có lỗi xảy ra, worker cập nhật trạng thái Job (`completed` hoặc `failed`), lưu tổng số dòng, số dòng đã nhập thành công và danh sách JSON lỗi cụ thể (bao gồm số dòng, email, và lý do lỗi), đồng thời gửi thông báo real-time qua WebSockets (`vip_import_status` event), thực hiện xóa file trên Supabase Storage, và gửi Acknowledge (Ack) xác nhận xử lý thành công cho RabbitMQ.
...
- Admin/Organizer nhận được thông báo real-time qua WebSockets ngay sau khi tiến trình xử lý import hoàn tất hoặc thất bại.
