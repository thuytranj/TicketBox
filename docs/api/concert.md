...
- **Request Body:**
  ...
  - `posterUrl` (string, optional): Đường dẫn ảnh poster.
  - `summary` (string, optional): Tóm tắt ngắn gọn sự kiện.
  - `tags` (string[], optional): Danh sách nhãn tag phân loại.
  - `svgStageMap` (string, optional): Bản đồ sơ đồ ghế ngồi dạng SVG.
...
- **Responses:**
  - **201 Created:** Tải lên thành công và trả về URL ảnh trên Cloudinary.
    ```json
    {
...
* **Ràng buộc nghiệp vụ:** Hệ thống kiểm tra xem concert đã phát sinh bất kỳ đơn đặt vé (booking) nào chưa.
  * Nếu chưa có booking: thực hiện xóa vật lý trong database, đồng thời thu hồi tất cả các khóa cache liên quan trong Redis.
