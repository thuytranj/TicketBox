...
| **POST** | `/auth/verify-reset-otp` | Public | Xác thực OTP reset và lấy Reset Token |
...
Xác thực mã OTP khôi phục mật khẩu nhận được qua email. Nếu thành công, trả về Reset Token tạm thời (TTL 5 phút) để dùng cho bước reset mật khẩu tiếp theo.
...
- **Responses:**
  - **200 OK:** Xác thực OTP thành công, trả về Reset Token.
    ```json
    {
      "resetToken": "093191f2f78d4403a156b093fa70951b"
    }
    ```
  - **401 Unauthorized:** Mã OTP reset bị sai hoặc đã hết hạn.
    ```json
    {
      "message": "Invalid OTP",
      "error": "Unauthorized",
    ...
    ```
  - **404 Not Found:** Email không tồn tại hoặc tài khoản chưa hoạt động.
    ```json
    {
      "message": "Email not found or account is not active",
      "error": "Not Found",
...
Đặt lại mật khẩu mới cho người dùng bằng mã Reset Token thu được từ bước verify OTP. Sau khi reset thành công, Reset Token sẽ bị thu hồi và tất cả Refresh Token của tài khoản này trên Redis sẽ bị xóa để buộc đăng xuất trên mọi thiết bị.
...
- **Request Body:**
  ...
  ```
  - `email` (string, required): Địa chỉ email.
  - `resetToken` (string, required): Mã Reset Token tạm thời.
...
- **Responses:**
  - **200 OK:** Cập nhật mật khẩu thành công.
    ...
    ```
  - **401 Unauthorized:** Reset Token sai hoặc đã hết hạn.
    ```json
    {
